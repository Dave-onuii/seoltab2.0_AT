...
@allure.epic("사용자 인증")
@allure.feature("로그인/로그아웃")
@allure.story("전체 로그인-로그아웃 플로우")
...
def test_login_logout(pages, test_account):
    ...
    # 4. 로그아웃 수행
    with allure.step("로그아웃 수행"):
        print("로그아웃을 시도합니다...")
        pages['my'].click_logout_button()

    # 5. 로그아웃 후 로그인 페이지가 보이는지 검증
    with allure.step("로그아웃 확인 - 로그인 페이지 노출 검증"):
...
def test_login_only(login_page, home_page, my_page, test_account):
    ...
    with allure.step("로그인 정보 검증"):
        ...
        allure.attach(logged_in_email, "로그인된 이메일", allure.attachment_type.TEXT)
...
def test_login_multiple_accounts(pages, account_name):
    ...
    allure.dynamic.title(f"{account_name} 계정 로그인 테스트")
    allure.dynamic.description(f"{account_name} 계정으로 로그인하여 정상 동작을 확인합니다.")

    print(f"\n[{account_name}] 계정으로 로그인을 시도합니다...")

    # 로그인
    with allure.step(f"{account_name} 계정으로 로그인"):
        allure.attach(account_name, "계정 이름", allure.attachment_type.TEXT)
        allure.attach(id_key, "사용자 이메일", allure.attachment_type.TEXT)
        pages['login'].login(id_key, pw_key)
